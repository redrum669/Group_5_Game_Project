...
// Only redraw the weapon slot (slot 1) â€?no flicker on weapon switch
...
// If portal is active, draw it â€?safe to call after any incremental update
void drawPortal();

// Update only slot 3 of the bottom bar (map name / portal prompt)
// nearPortal=true  â†?shows "[D] ENTER" prompt
// nearPortal=false â†?shows normal map name
