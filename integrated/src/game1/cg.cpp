...
// ==================== Drawing helpers =============================
// ST7789 has no circle/line primitives â€?implement locally.
...
// Filled circle â€?midpoint algorithm using horizontal scanlines
...
// Circle outline â€?midpoint algorithm using individual pixels
...
// ------------------------------------------------------------------
// Draw starfield â€?called once at scene start, not every frame
...
static void drawStoryInit() {
    ...
    // Faint skip hint â€?will be replaced with "[A] Continue" when done
...
static void animMissiles(uint32_t elapsed) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (missiles[i].step >= MISSILE_STEPS) {
            // Impact â€?erase trail and trigger explosion
            for (int j = 0; j < MTRAIL; j++)
...
// Master Phase 2 coordinator â€?called every frame
...
bool cgUpdate() {
    ...
    if (cgPhase == 0) {
        cgPhase     = 1;       // A â†?skip story text, go to invasion scene
        cgStartTime = millis();
        cgDirty     = true;
        return false;
    }
    if (cgPhase == 1) {
        cgPhase     = 2;       // A â†?skip static scene, start destruction anim
        cgStartTime = millis();
        cgDirty     = true;
        return false;
    }
    if (cgPhase == 2) {
        cgPhase     = 3;       // A â†?skip destruction, go to lore text
        cgStartTime = millis();
        cgDirty     = true;
        return false;
    }
